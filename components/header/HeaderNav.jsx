...
import { premiumBoxStore, showMoreStore } from '@/store';

function HeaderNav({ user }) {
    const currentRoute = usePathname();
    const premiumBoxActive = premiumBoxStore((state) => state.premiumBoxActive)
    const showMoreActive = showMoreStore((state) => state.showMoreActive)

    return (
        <>
            <nav className={style.nav}>
                <Link href={'/'} className={`${currentRoute === '/' ? style.active : ''}`}>
                ...
                <Link href={'/explore'} className={`${currentRoute === '/explore' ? style.active : ''}`}>
                    <div className={style.container}>
                ...
                <Link href={'/notifications'} className={`${currentRoute === '/notifications' ? style.active : ''}`}>
                    <div className={style.container}>
                ...
                <Link href={'/messages'} className={`${currentRoute === '/messages' ? style.active : ''}`}>
                    <div className={style.container}>
                ...
                <Link href={'/' + user[0]["login"].username + '/lists'} className={`${currentRoute === '/' + user[0]["login"].username + '/lists' ? style.active : ''}`}>
                    <div className={style.container}>
                        <svg viewBox="0 0 24 24"><path d="M3 4.5C3 3.12 4.12 2 5.5 2h13C19.88 2 21 3.12 21 4.5v15c0 1.38-1.12 2.5-2.5 2.5h-13C4.12 22 3 20.88 3 19.5v-15zM5.5 4c-.28 0-.5.22-.5.5v15c0 .28.22.5.5.5h13c.28 0 .5-.22.5-.5v-15c0-.28-.22-.5-.5-.5h-13zM16 10H8V8h8v2zm-8 2h8v2H8v-2z"></path></svg>
                        <span>Listeler</span>
                    </div>
                </Link>
                <Link href={'#'} onClick={premiumBoxActive}>
                    <div className={style.container}>
                        <svg viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"></path></svg>
                        <span>Premium</span>
                    </div>
                </Link>
                <Link href={'/' + user[0]["login"].username} className={`${currentRoute === '/' + user[0]["login"].username ? style.active : ''}`}>
                    <div className={style.container}>
