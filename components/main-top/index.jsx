...
function MainTop({ isNP, isMain, isUserPage, name, isStatus }) {
    const router = useRouter()

    return (
        <div className={`${style.mainTop} ${isUserPage && style.isUserPage} ${isStatus && style.isStatus}`}>
            {isUserPage &&
            ...
            {isMain &&
                <span className={style.isMainIcon}>
                ...
                <nav className={style.nav}>
                    <div>
                        {isNP ?
                            <Link href={'/'}>Tümü</Link>
                            :
                            <Link href={'/'} className={style.navActive}>Sana özel</Link>
                        }
                    </div>
                    <div>
                        {isNP ?
                            <Link href={'/'}>Onaylanmış</Link>
                            :
                            <Link href={'/'}>Takip edilenler</Link>
                        }
                    </div>
                    {isNP &&
                        <div>
                            <Link href={'/'}>Bahsedenler</Link>
                        </div>
                    }
                </nav>
            }
